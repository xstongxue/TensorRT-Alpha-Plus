#include "/data02/xs/code/tensorrt-alpha/webapi_rtdetr/apps/rtdetr_web.hpp"
#include <queue>
...
namespace RTDETRWEB{
    ...
    using ControllerImpl = InferControllerWEB<cv::Mat, BoxArray, tuple<string, int>, AffineMatrix>;

    class InferImpl : public Detect, public ControllerImpl{
    public:
        ...
        vector<shared_future<BoxArray>> commits_getBoxes(const vector<cv::Mat>& images) override{
            return ControllerImpl::commits_getBoxes(images);
        }

        std::shared_future<BoxArray> commit_getBoxes(const cv::Mat& image) override{
            return ControllerImpl::commit_getBoxes(image);
        }
