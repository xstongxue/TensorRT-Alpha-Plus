...
namespace yolo
{
    ...
    public:
        ...
        void setBoxes(const std::vector<std::tuple<std::string, float, float, float, float, float>>& boxes);
        std::vector<std::tuple<std::string, float, float, float, float, float>> getBoxes() const;
    
    protected:
        std::unique_ptr<nvinfer1::IRuntime> m_runtime; // 将runtime作为类成员
    ...
    protected:
        ...
        std::vector<std::tuple<std::string, float, float, float, float, float>> m_boxes;
