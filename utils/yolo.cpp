...
yolo::YOLO::YOLO(const utils::InitParameter& param) : m_param(param)
{
    ...
    m_boxes.resize(param.batch_size);
}
...
void yolo::YOLO::setBoxes(const std::vector<std::tuple<std::string, float, float, float, float, float>>& boxes) {
    this->m_boxes = boxes;
}

std::vector<std::tuple<std::string, float, float, float, float, float>> yolo::YOLO::getBoxes() const
{
    return this->m_boxes;
}
...
{
    // 清除设备内存
    cudaError_t err = cudaMemset(m_output_objects_device, 0, sizeof(float) * m_param.batch_size * (1 + 7 * m_param.topK));
    if (err != cudaSuccess) {
    ...
    for (size_t bi = 0; bi < m_param.batch_size; bi++) {
        m_objectss[bi].clear();
        std::cout << "Cleared objectss[" << bi << "], size: " << m_objectss[bi].size() << std::endl;
    }
}